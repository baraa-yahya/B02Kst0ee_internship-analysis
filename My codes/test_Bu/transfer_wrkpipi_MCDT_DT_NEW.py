'''
Author: Michele Atzeni, matzeni@cern.ch
Script to transfer the rkpipi weights from MCDT to DT
'''
import sys
import os
import pandas as pd
...
parser.add_argument("-f", "--MC_file_name", dest="MC_file_name", required=True, help="Choose MC_file_name")    
parser.add_argument("-m", "--mag", dest="mag", required=True, help="Choose mag", choices=['MagDown',"MagUp"])
parser.add_argument("-y", "--year", dest="year", required=True, help="Choose year", choices=['2011',"2012",'2015','2016','2017','2018'])

args = parser.parse_args()
MC_file_name = args.MC_file_name
mag = args.mag
year = args.year

# ifile_MCDT = f"/sps/lhcb/yahya/Internship/test_Bu/eso_files_bu/Bu2Kpipiee-MCDT-{year}-{mag}-RKpipiWeights.root"
...
# mc_branches = ['runNumber', 'eventNumber', 'nCandidate', 'Bp_TRUEETA', 'Bp_TRUEPHI', 'Bp_TRUEP_E', 'kpipi_weight']
mc_branches = ['runNumber', 'eventNumber', 'nCandidate', 'Bp_TRUEETA', 'Bp_TRUEPHI', 'Bp_TRUEP_E', 
               ...
               'ctl_lhcb', 'ctk_lhcb', 'phi_lhcb']

# dfMCDT = ur.open(ifile_MCDT)["MCDecayTree"].arrays(expressions=['runNumber','eventNumber', 'nCandidate', 'Bp_TRUEETA', 'Bp_TRUEPHI', 'Bp_TRUEP_E', 'kpipi_weight', 'ctl_lhcb', 'ctk_lhcb', 'phi_lhcb'], library='pd')
...
#                     'B_eta', 'B_phi', 'B_TRUEP_E'
#                 ]

DT_branches = [
    'runNumber', 'eventNumber',
    'B_eta', 'B_phi', 'B_TRUEP_E', 'ctl_lhcb', 'ctk_lhcb', 'phi_lhcb', 'B_DTF_PV_B_M_0',
]
...
for i in range(len(mult_cand_events)):
    ...
    if len(matched_indices) == 0:
        ...
        if (len(set(choices)) == 1):
            ...
            dfMCDT.at[remaining_index, "ctl_lhcb"] = 0
