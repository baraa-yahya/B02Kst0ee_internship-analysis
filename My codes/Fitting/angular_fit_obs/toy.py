...
import progressbar
import b2kstll
############### Calculation of Angular observables from SM #################################
import flavio
...
obs = zfit.Space("obs1", limits=(-1, 1)) * zfit.Space("obs2", limits=(-1, 1)) * zfit.Space("obs3", limits=(-np.pi, np.pi))
...
loss = zfit.loss.UnbinnedNLL(model, sampler)#data)


minimizer = zfit.minimize.Minuit()

result = minimizer.minimize(loss)
fit_result = result.params
print(result)
# b2kstll.plot.plot_toys(model=model, data = sampler, suffix ="test_lhcb")

# b2kstll.plot.plot_distributions_using_partial_integrals(model=model, data = sampler, suffix ="toys",with_pulls=False)
